-----------------------------------------------------------------------------------------------
-- Client Lua Script for VikingRecall
...
-----------------------------------------------------------------------------------------------
-- VikingRecall Module Definition
-----------------------------------------------------------------------------------------------
local VikingRecall = {} 
...
function VikingRecall:Init()
    Apollo.RegisterAddon(self, nil, nil, {"Vikingbar"})
end
...
-- VikingRecall OnLoad
-----------------------------------------------------------------------------------------------
function VikingRecall:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("VikingRecall.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
	
	Apollo.RegisterEventHandler("ActionBarReady", "OnDocumentReady", self)
end

function VikingRecall:OnDocumentReady(wndParent)
	...
	-- load our forms
    self.wndMain = Apollo.LoadForm(self.xmlDoc, "VikingRecallForm", wndParent, self)
...
-----------------------------------------------------------------------------------------------
-- VikingRecall Functions
-----------------------------------------------------------------------------------------------
-- Define general functions here

	
function VikingRecall:RefreshDefaultCommand()
...
function VikingRecall:ResetDefaultCommand()
...
function VikingRecall:OnGenerateTooltip(wndControl, wndHandler, tType, arg1, arg2)
...
-----------------------------------------------------------------------------------------------
-- VikingRecallForm Functions
-----------------------------------------------------------------------------------------------

function VikingRecall:OnRecallOptionToggle(wndHandler, wndControl, eMouseButton)
...
function VikingRecall:GenerateBindList()
...
function VikingRecall:OnRecallBtn(wndHandler, wndControl)
	self.wndMain:FindChild("RecallActionBtn"):SetContentId(wndControl:GetData())
	self.wndMenu:Show(false)
	GameLib.SetDefaultRecallCommand(wndControl:GetData())
end

function VikingRecall:OnCloseBtn()
	self.wndMenu:Show(false)
end

function VikingRecall:OnChangeWorld()
	self.bHaveNeighborhoods = false
	self.wndMenu:Show(false)
end

function VikingRecall:OnGuildResult(guildCurr, strName, nRank, eResult) -- guild object, name string, Rank, result enum
...
function VikingRecall:OnAbilityBookChange()
	self.wndMenu:Show(false)
	-- Process on the next frame.
	Apollo.CreateTimer("RefreshRecallTimer", 0.001, false)
end

function VikingRecall:OnTutorial_RequestUIAnchor(eAnchor, idTutorial, strPopupText)
...
-----------------------------------------------------------------------------------------------
-- VikingRecall Instance
-----------------------------------------------------------------------------------------------
local VikingRecallInst = VikingRecall:new()
VikingRecallInst:Init()
